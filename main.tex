...
\textbf{ \Large Forecasting Wind Velocity Using an Autoregressive Model}\\[1.5cm]
...
Autoregression is a well-established method for short-term (typically at most 2 days) time-series forecasting of wind velocity, but the performance of the model depends on the timescale characteristics of the winds.
...
Subsequently, an autoregressive model is implemented and its performance is compared, in terms of a prediction error, to the performances of two simpler predictive models: a persistence model and a random sample model.
...
Indeed, performance results show that, while the autoregressive model performs consistently superior to the alternative models, it reaches a plateau in prediction error over approximately that timescale.
...
The performance of the autoregressive model is then compared, over various prediction timescales, to the performances of two simpler predictive models: a persistence model and a random sample model.
...
%%TODO%% note that this differs from the literature since we predict the velocity rather than speed
...
\caption{Comparison of RMS prediction errors incurred by the persistence (blue curve), random sample (red), and autoregressive (green) models.
...
From this plot, we see that, beyond about 1 day, the autoregressive model performs consistently superior to both the persistence and random sample models.
...
This suggests that, even though the system has ``forgotten'' about the previous wind conditions, the autoregressive model is still able to achieve predictions that are, on average, \textit{slightly} more accurate than the totally independent predictions produced by the alternative models.
...
The RMS prediction errors incurred in each season by the autoregressive model are plotted in \figref{fig:SeasonalRMS}.
...
\caption{Seasonal dependence of RMS prediction errors incurred by the autoregressive model.
The vertical dashed lines indicate 2 and 5.5 days.}
\label{fig:SeasonalRMS}
\end{figure}

The RMS prediction errors incurred under daytime and nighttime conditions by the autoregressive model are plotted in \figref{fig:DiurnalRMS}.
...
\caption{Time-of-day dependence of RMS prediction errors incurred by the autoregressive model.
...
Subsequently, we compared, over various timescales and in terms of an average prediction error, the performance of the autoregressive model to the performances of two simpler predictive models: a persistence model and a random sample.
...
Indeed, results show that, while the autoregressive model performs consistently superior to the simpler alternative models, it reaches a plateau in prediction error over approximately that timescale (see \figref{fig:ComparisonRMS}).
The autoregressive model was also found to yield more accurate predictions in the Summer than in the Winter (see \figref{fig:SeasonalRMS}), which suggests that wind conditions in the Summer may be more consistent and ``predictable'' as a result.
...
For example, a regime-switching approach, in which different autoregression coefficients are used for each season or for each time of day, can be expected to achieve improved performance under these various conditions.
