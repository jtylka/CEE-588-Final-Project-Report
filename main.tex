...
Autoregression is a well-established empirical method for near-term (typically at most two days) forecasting of wind velocity, but the parameters of the prediction model (i.e., the autoregression coefficients) should be determined based on a characterization of past wind data.
Using a time-series dataset of wind velocities, relevant timescales are determined over which past wind velocities are useful predictors of future data.
Subsequently, an autoregressive model is implemented and its performance is evaluated, in terms of a prediction error, and compared to that of two simpler predictive models: a persistence model and a random sample.
...
% Q1: what are the relevant timescales? i.e., how far into the future must one travel to "forget" about the current wind conditions?
...
Here, we take $N_\text{in}$ and $N_\text{out}$ to correspond to 10 and 30 days, respectively. %%TODO%% redo with N_in = 2 days
